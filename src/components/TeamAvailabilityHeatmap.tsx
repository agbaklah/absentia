...
export function TeamAvailabilityHeatmap({
  ...
  const filteredEntries = useMemo(
    () => (entries.data ?? []).filter((e) => empIds.has(e.employee_id) && e.status !== "rejected"),
    [entries.data, empIds],
  );
    ...
    <Card className="overflow-hidden">
        ...
        <table className="w-full text-[10px]">
                ...
                return (
                  <th
                    ...
                    className={cn(
                      ...
                      wknd || hol ? "bg-muted/60 text-muted-foreground" : "text-muted-foreground",
                    )}
                  >
          ...
          <tbody>
            ...
            {groupedByTeam.length === 0 && (
              <tr>
                <td colSpan={days + 2} className="px-3 py-8 text-center text-muted-foreground">
      ...
      <div className="flex flex-wrap items-center gap-x-3 gap-y-1 border-t px-3 py-2">
        ...
        <div className="inline-flex items-center gap-1">
          ...
          <span className="text-[10px] text-muted-foreground">Weekend / holiday</span>
        </div>
      </div>
    </Card>
  );
        ...
        return (
          <tr key={emp.id} className="hover:bg-muted/10">
            ...
            {Array.from({ length: days }, (_, i) => {
              ...
              return (
                <td key={i} className={cn("border-b p-0", (wknd || hol) && "bg-muted/40")}>
                  <div
                    className={cn(
