...
export const completePasswordChange = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const { data: profile, error: profileErr } = await supabaseAdmin
      .from("profiles")
      .select("id, force_password_change, temp_password_hash, temp_password_expires_at")
    ...
    if (profile.force_password_change && profile.temp_password_hash) {
      ...
      if (
        ...
        return {
          error: "Your temporary password is no longer valid. Please contact your administrator.",
        } as const;
      }
      ...
      if (compareHashes(newHash, profile.temp_password_hash)) {
        return {
          error: "You cannot reuse the temporary password as your permanent password.",
        } as const;
      }
    }
    ...
    const { error: clearErr } = await supabaseAdmin
      ...
      .eq("id", profile.id);
    if (clearErr) {
      console.warn("[completePasswordChange] Could not clear flags:", clearErr.message);
    }
