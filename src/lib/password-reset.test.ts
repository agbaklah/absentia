...
describe("generateTempPassword", () => {
  ...
  it("excludes visually confusing characters (0, O, o, 1, l, I)", () => {
    ...
    for (let i = 0; i < 50; i++) {
      const pw = generateTempPassword();
      assert.ok(!confusing.test(pw), `confusing char found in "${pw}" on iteration ${i}`);
    }
  });
  ...
  it("uses only characters from the allowed set", () => {
    ...
    for (let i = 0; i < 50; i++) {
      const pw = generateTempPassword();
      assert.ok(allowed.test(pw), `disallowed character in "${pw}" on iteration ${i}`);
    }
  });
  ...
  it("satisfies the password strength policy (score 4 / Strong)", () => {
    ...
    for (let i = 0; i < 50; i++) {
      ...
      assert.equal(met, 5, `password "${pw}" only met ${met}/5 strength checks`);
    }
  });
});
...
describe("compareHashes", () => {
  ...
  it("handles single-character difference at various positions", () => {
    ...
    for (let i = 0; i < base.length; i++) {
      const modified = base.slice(0, i) + "b" + base.slice(i + 1);
