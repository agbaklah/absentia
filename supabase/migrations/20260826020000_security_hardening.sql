...
BEGIN
  -- Only apply to end-user sessions. Server functions use the service role
  -- (claims.role = 'service_role') and migrations have no claims at all.
  IF claims IS NOT NULL AND COALESCE(claims->>'role', '') <> 'service_role' THEN
...
-- 5. (Removed) The original block referenced NEW.role inside an RLS policy,
--    which Postgres rejects. The same guarantee — only super admins may create
--    admin/super_admin (and cfo) profiles — is already enforced by the
--    prevent_unauthorized_admin_roles trigger from 20260818000001.
