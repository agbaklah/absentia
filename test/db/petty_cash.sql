-- DB-level tests for the petty cash state machine + RLS. Runs as a single
-- transaction that is always rolled back. Impersonates users by setting the
-- JWT claims PostgREST would set. Each check raises on failure.
BEGIN;
...
END $$;
-- Expect a statement to fail with a message containing _needle.
